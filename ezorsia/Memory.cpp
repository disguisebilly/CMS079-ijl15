...
DWORD* Memory::getAddress(DWORD dwOriginAddress, std::initializer_list<int> offset)
{
	auto address = reinterpret_cast<DWORD*>(dwOriginAddress);
	auto it = offset.begin();
	while (it != offset.end()) {
		if (!*address)
			return NULL;
		address = reinterpret_cast<DWORD*>(*address + *it);
		++it;
	}
	return address;
}
